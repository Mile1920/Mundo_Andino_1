...
using UnityEditor;
using UnityEngine;
using UnityEditor.AssetImporters;   // ← Esto soluciona TextureImporterCompression
...
namespace UnityStandardAssets.ImageEffects
{
    ...
    class ColorCorrectionLookupEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                var ccl = target as ColorCorrectionLookup;
                if (ccl != null)
                {
                    tempClutTex2D = AssetDatabase.LoadAssetAtPath<Texture2D>(ccl.basedOnTempTex);
                }
            }
            ...
            if (tex && (target as ColorCorrectionLookup).basedOnTempTex != AssetDatabase.GetAssetPath(tex))
            {
                ...
                var ccl = target as ColorCorrectionLookup;
                if (ccl != null && !ccl.ValidDimensions(tex))
                {
            ...
            if ((target as ColorCorrectionLookup).basedOnTempTex != "")
            {
                ...
                var ccl = target as ColorCorrectionLookup;
                Texture2D t = AssetDatabase.LoadAssetAtPath<Texture2D>(ccl.basedOnTempTex);
